...
class AppBridge {
  ...
  startAuth() {
    return [
      (req, res, next) => {
        req.session.afterAuthRedirectTo = req.query.after_auth_redirect_to || '/';
        next();
      },
      async (req, res, next) => await this.developerOauth.authenticate(req, res, next),
      (req, res) => {
  ...
  callback() {
    ...
    return [
      async (req, res, next) => await this.developerOauth.callback(req, res, next),
      async (req, res) => {
        const tokenData = req.session.accessTokens.slice(-1)[0];
        await this.tokenStore.save(this.developerOauth.config.client_id, tokenData.merchantId, tokenData.performerId, tokenData);
