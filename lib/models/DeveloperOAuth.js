...
class DeveloperOAuth {
  ...
  async authenticate(req, res, next) {
    try {
      ...
      if (this.config.ensure_login_session) {
        ...
        if (this.isSsoSessionExpired(req, currSsoSessionId)) {
  ...
  isSsoSessionExpired(req, currSsoSessionId) {
    const { prevSsoSessionId } = req.session;
    return !prevSsoSessionId || !currSsoSessionId || prevSsoSessionId !== currSsoSessionId;
  }
