...
class DeveloperOAuth {
  ...
  async authenticate(req, res, next) {
    try {
      ...
      if (this.config.ensure_login_session) {
        ...
        const currentSession = (currentStaff || {}).public_session_id_hash;
        if (this.isSsoSessionExpired(req, currentSession)) {
          await this.regenerateSession(req);
          req.session.prevSsoSessionId = currentSession;
  ...
  isSsoSessionExpired(req, currentSession) {
    const prevSession = req.session.prevSsoSessionId;
    return !prevSession || !currentSession || prevSession !== currentSession;
  }
