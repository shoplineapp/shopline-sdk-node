...
const MemoryTokenStore = require('./tokenStores/MemoryTokenStore');
const MemorySessionStore = require('./sessionStores/MemorySessionStore');
...
class DeveloperOAuth {
  constructor({
    ...
    embeddedAppSessionStore = new MemorySessionStore(),
    ...
    this.config = {
      ...
      embedded_app_path: embeddedAppPath,
      scope,
    };
    ...
    this.embeddedAppSessionStore = embeddedAppSessionStore;
  }
  ...
  async embeddedAppCallback(req, res, next) {
    try {
      const oauthRes = await this.exchangeToken(req.query.code);
      const tokenInfoRes = await this.tokenInfo(oauthRes.data.access_token);
      const handle = (tokenInfoRes.data.merchant || {}).handle;
      const staffId = (tokenInfoRes.data.staff || {})._id;
      const returnUrl = `https://admin.sw90.shoplinestg.com/admin/${handle}${this.config.embedded_app_path}`;

      await this.tokenStore.save(staffId, {
        ...oauthRes.data,
        ...tokenInfoRes.data
      });
  ...
  async verifySessionToken(req, res, next) {
    ...
    const embeddedAppSessionId = this.getEmbeddedAppSessionId(sessionTokenPayload);

    res.locals.currentMerchantId = sessionTokenPayload.data.merchant_id;
    res.locals.performerId = sessionTokenPayload.data.staff_id;
    // res.locals.currentToken = null;
    // res.locals.performerMerchantIds = [];

    req.embeddedAppSession = await this.embeddedAppSessionStore.fetch(embeddedAppSessionId);
    next();
    await this.embeddedAppSessionStore.save(req.embeddedAppSession);
  }

  async fetchTokenInfo(req, res, next) {
    const tokenInfo = await this.tokenStore.fetch(res.locals.performerId);
    if (!tokenInfo) {
      return next();
    }
    // TODO: refresh token if expired
    res.locals.currentToken = tokenInfo.access_token;
    res.locals.performerMerchantIds = tokenInfo.staff.merchant_ids;
    return next();
  }

  getEmbeddedAppSessionId(payload) {
    if(!payload.data.client_id || !payload.data.merchant_id || !payload.data.staff_id) {
      throw Error('Invalid session token');
    }

    return `${payload.data.client_id}-${payload.data.merchant_id}-${payload.data.staff_id}`
  }

  decodeSessionToken(sessionToken) {
    try {
      const payload = jwt.verify(sessionToken, this.config.client_secret);
      return payload;
    } catch (e) {
