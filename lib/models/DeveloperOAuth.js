...
class DeveloperOAuth {
  ...
  async callback(req, res, next) {
    try {
      ...
      if (!this.config.ensure_login_session) {
        await this.regenerateSession(req, true);
      } else {
        ...
        if (this.isSsoSessionChanged(req, currSsoSessionId)) {
          await this.regenerateSession(req, false);
  ...
  async authenticate(req, res, next, {signOutUrl, skipAuthOnNoSession, redirectUrl} = {}) {
    try {
      if (!this.config) {
      ...
      if (this.config.ensure_login_session) {
        ...
        if (this.isSsoSessionChanged(req, currSsoSessionId)) {
          ...
          req.session.prevSsoSessionId = currSsoSessionId;
          req.session.afterAuthRedirectTo = redirectUrl ?? res.locals.currentURL;
          this.log('request', 'trace', {
            message: 'sso session changed, redirect to oauth authorize url',
      ...
      if (skipAuthOnNoSession && signOutUrl) {
        this.log('request', 'trace', {
          message: 'Redirect to remove session',
          locals: res.locals,
          session: req.session,
        });
  ...
  constructOAuthAuthorizeUrl({ merchantId, signOutUrl } = {}) {
    const oauthParams = {
    ...
    if (signOutUrl) {
      oauthParams.sign_out_url = signOutUrl;
    }
  ...
  async regenerateSession(req, persistData = false) {
    this.log('request', 'trace', {
